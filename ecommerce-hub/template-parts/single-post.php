...
<article>
	...
	<?php if( get_theme_mod( 'ecommerce_hub_single_post_date',true) != '' || get_theme_mod( 'ecommerce_hub_single_post_author',true) != '' || get_theme_mod( 'ecommerce_hub_single_post_comment_no',true) != '' || get_theme_mod( 'ecommerce_hub_single_post_time',true) != '') { ?>
	    <div class="post-info py-1 px-2 mb-2">
	      	<?php if( get_theme_mod( 'ecommerce_hub_single_post_date',true) != '') { ?>
	        	<i class="<?php echo esc_attr(get_theme_mod('ecommerce_hub_singlepost_date_icon',"fa fa-calendar pe-2" )); ?>"></i><a href="<?php echo esc_url( get_day_link( $archive_year, $archive_month, $archive_day)); ?>"><span class="entry-date ps-1 pe-2"><?php echo esc_html( get_the_date() ); ?></span><span class="screen-reader-text"><?php echo esc_html( get_the_date() ); ?></span></a>
	      	<?php }?>
	      	<?php if( get_theme_mod( 'ecommerce_hub_single_post_author',true) != '') { ?>
	        	<i class="<?php echo esc_attr(get_theme_mod('ecommerce_hub_singlepost_author_icon',"fa fa-user pe-2" )); ?>"></i><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><span class="entry-author ps-1 pe-2"> <?php the_author(); ?></span><span class="screen-reader-text"><?php the_author(); ?></span></a>
	      	<?php }?>
	      	<?php if( get_theme_mod( 'ecommerce_hub_single_post_comment_no',true) != '') { ?>
	        	<i class="<?php echo esc_attr(get_theme_mod('ecommerce_hub_single_post_comment_icon',"fa fa-comments pe-2" )); ?>"></i><span class="entry-comments ps-1 pe-2"> <?php comments_number( __('0 Comments','ecommerce-hub'), __('0 Comments','ecommerce-hub'), __('% Comments','ecommerce-hub') ); ?></span> 
	      	<?php }?>
	      	<?php if( get_theme_mod( 'ecommerce_hub_single_post_time',true) != '') { ?>
	        	<span class="entry-comments me-2"><i class="<?php echo esc_attr(get_theme_mod('ecommerce_hub_single_post_time_icon',"fa fa-clock pe-2" )); ?>"></i> <?php echo esc_html( get_the_time() ); ?></span>
	        <?php }?>
	    </div>
	<?php }?>
	<?php if( get_theme_mod( 'ecommerce_hub_single_post_image',true) != '' && get_theme_mod('ecommerce_hub_post_featured_image','in-content') == 'in-content') { ?>
		...
		<?php } ?>
	<?php }?>
	<?php if( get_theme_mod( 'ecommerce_hub_single_post_category',true) != '') { ?>
		<div class="single-post-category mt-3">
			<span class="category"><?php esc_html_e('Categories:','ecommerce-hub'); ?></span>
			<?php the_category(); ?>
		</div>
	<?php }?>
